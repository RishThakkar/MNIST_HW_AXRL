...
{
   FILE *file = fopen("binary_weights_FP_scaled_new.txt", "w");
    if (file == NULL) 
    {
    ...
    freopen("binary_weights_FP_scaled_new.txt", "w", stdout);
...
{
   FILE *file = fopen("binary_image0_real.txt", "w");
    if (file == NULL) 
    {
        printf("Error opening file!\n");
        return;
    ...
    freopen("binary_image0_real.txt", "w", stdout);
...
int main()
{
    ...
    int imglen = sizeof(image0_real) / sizeof(image0_real[0]);
    imagearray(image0_real, imglen);

    int len = sizeof(weights_bias_FP_scaled) / sizeof(weights_bias_FP_scaled[0]);
    // weightarray(weights_bias_FP_scaled, len);
